...
Authenticates with Hikvision devices and retrieves encrypted data
"""
...
class HikvisionClient:
    ...
    def login(self):
        """
        Perform login to Hikvision device
        ...
        url = f"{self.protocol}://{self.ip}/ISAPI/Security/sessionLogin/capabilities?username={self.username}"
        ...
        # Step 4: Login
        print("[*] Step 4: Performing login...")
        timestamp = int(time.time() * 1000)
        random_num = secrets.randbelow(100000000)
        login_url = f"{self.protocol}://{self.ip}/ISAPI/Security/sessionLogin?timeStamp={timestamp}&random={random_num}"
...
<SessionLogin>
    ...
    <sessionIDVersion>2</sessionIDVersion>
        ...
        # Check response
        if '<statusValue>200</statusValue>' in resp.text or '<statusString>OK</statusString>' in resp.text:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Hikvision Device Client')
    ...
    # Create client
    client = HikvisionClient(args.ip, args.username, args.password, use_https=args.https)
