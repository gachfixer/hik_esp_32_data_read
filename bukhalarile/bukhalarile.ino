...
const char* OTA_FIRMWARE_URL = "https://api.scantrack.ndovupay.org/bukhlarile.bin";
...
const char* HIK_IP = "192.168.0.182";
const char* HIK_USERNAME = "admin";
const char* HIK_PASSWORD = "dev@spa!";
const bool USE_HTTPS = false;

static unsigned long lastHeartbeat = 0;
static unsigned long lastGachfixSend = 0;
const unsigned long GACHFIX_INTERVAL = 10UL * 60 * 1000;  // 10 minutes
...
int searchPosition = 0;     // Pagination position for event fetching
...
/**
 * Send current time to gachfix server
 * GET https://gachfix.co.ke/lasttime.php?time=<currentTime>&device=<deviceId>
 */
void sendLastTimeToGachfix() {
  String currentTime = getISOTimestamp();

  HTTPClient http;
  String url = "https://gachfix.co.ke/lasttime.php?time=" + currentTime + "&device=" + deviceId;
  ...
  if (httpCode == 200) {
    Serial.println("[+] Sent lastEventTime to gachfix OK");
  } else {
    Serial.printf("[!] Gachfix update failed, HTTP %d\n", httpCode);
  }
 ...
 * Uses mktime() for proper second/minute/hour/day rollover
 */
String incrementTime(const String& timeStr) {
  ...
  // Extract timezone suffix (e.g., "+08:00")
  String tzSuffix = timeStr.substring(19);

  // Use struct tm and mktime for proper rollover
  struct tm t = {0};
  t.tm_year = year - 1900;
  t.tm_mon = month - 1;
  t.tm_mday = day;
  t.tm_hour = hour;
  t.tm_min = minute;
  t.tm_sec = second + 1;  // Add 1 second
  mktime(&t);             // Normalize (handles all rollovers)

  char newTime[30];
  snprintf(newTime, sizeof(newTime), "%04d-%02d-%02dT%02d:%02d:%02d%s",
 ...
 * Uses position-based pagination to avoid skipping events with the same timestamp.
 * Only advances lastEventTime after ALL events in the time window are processed.
 */
void fetchAttendanceEvents(int maxResults = 30) {
  if (!isLoggedIn) {
    Serial.println("[!] Not logged in");
    return;
  }
  ...
  // End time: 24 hours in the future (ensures all events are captured)
  struct tm timeinfo;
  localtime_r(&now, &timeinfo);
  timeinfo.tm_hour += 24;
  ...
  Serial.printf("[*] Fetching events from: %s (pos=%d) to: %s\n",
                lastEventTime.c_str(), searchPosition, endTime.c_str());

  // Create JSON payload - oldest first with position-based pagination
  DynamicJsonDocument doc(1024);
  JsonObject cond = doc.createNestedObject("AcsEventCond");
  cond["searchID"] = generateUUID();
  cond["searchResultPosition"] = searchPosition;
  cond["maxResults"] = maxResults;
  cond["major"] = 0;
  cond["minor"] = 0;
  ...
  if (httpCode == 200) {
    ...
    int totalMatches = acsEvent["totalMatches"] | 0;
    Serial.println("----------------------------------------");
    Serial.printf("[*] Page: pos=%d, got=%d, total=%d\n",
                  searchPosition, numMatches, totalMatches);

    // Safety: if no matches returned, reset position
    if (numMatches == 0) {
      if (searchPosition > 0) {
        Serial.println("[*] No more events at current position, resetting");
        searchPosition = 0;
      }
      http.end();
      return;
    ...
    JsonArray events = acsEvent["InfoList"];
    int sent = 0;
    int processed = 0;  // Events iterated (including skipped system events)
    int skippedSystem = 0;  // Count of system events (no employeeNo) in this batch
    String lastEventTimeInBatch = "";
    bool sendFailed = false;

    for (JsonObject event : events) {
      const char* eventTime = event["time"] | "";

      // Track last event time in batch (any event type) for time advancement
      if (strlen(eventTime) > 0) {
        lastEventTimeInBatch = String(eventTime);
      }

      const char* employeeNo = event["employeeNoString"] | "";

      // Skip events without employee number (system events)
    ...
    if (sendFailed) {
      // Advance position to the failed event so we retry from there
      searchPosition += processed;
      Serial.printf("[*] Send failed, will retry from pos=%d\n", searchPosition);
    } else {
      // All events in batch processed successfully
      searchPosition += numMatches;

      // Check if we've processed all events in this time window
      if (searchPosition >= totalMatches) {
        if (lastEventTimeInBatch.length() > 0) {
          lastEventTime = incrementTime(lastEventTimeInBatch);
          Serial.printf("[*] All %d events processed, new start: %s\n",
                        totalMatches, lastEventTime.c_str());
        }
        searchPosition = 0;  // Reset for next time window
      } else {
        Serial.printf("[*] More events remaining (%d/%d), continuing next cycle\n",
                      searchPosition, totalMatches);
      }
    }

    if (sent > 0 || numMatches > 0) {
      Serial.printf("[*] Batch: %d sent, %d skipped, %d in page, %d total\n",
                    sent, skippedSystem, numMatches, totalMatches);
    }
...
void initiateValues(bool isReconnect){
  ...
  if (time(nullptr) > 1000000000) {
    ...
    if (!isReconnect) {
      ...
      searchPosition = 0;  // Reset pagination when starting fresh
    ...
    // Send current time to gachfix on startup
  sendLastTimeToGachfix();
  lastGachfixSend = millis();
...
void loop() {
  ...
  // Send current time to gachfix every 10 minutes
  if (currentMillis - lastGachfixSend >= GACHFIX_INTERVAL) {
    sendLastTimeToGachfix();
    lastGachfixSend = currentMillis;
  }
