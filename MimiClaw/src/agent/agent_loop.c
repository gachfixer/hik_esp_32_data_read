...
static void agent_loop_task(void *arg)
{
    ...
    /* Allocate large buffers — prefer PSRAM, fall back to internal RAM */
    ...
    if (!system_prompt) system_prompt = calloc(1, MIMI_CONTEXT_BUF_SIZE);
    if (!history_json)  history_json  = calloc(1, MIMI_LLM_STREAM_BUF_SIZE);
    if (!tool_output)   tool_output   = calloc(1, TOOL_OUTPUT_SIZE);
